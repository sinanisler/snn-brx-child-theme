...
class SNN_Video_Player_Element extends Element {
    ...
    /**
     * Set the controls for the element in the builder.
     */
    public function set_controls() {
        ...
        $this->controls['chapters'] = [
            ...
            'titleProperty' => 'title',
            'default'       => [
                ['title' => 'Bunny Wakes Up', 'time'  => '0:30'],
                ['title' => 'Butterfly Chase', 'time'  => '2:45'],
                ['title' => 'The Bridge', 'time'  => '5:20'],
            ],
            'fields'        => [
        ...
        $this->controls['slider_track_color'] = [
            ...
            'type'    => 'color',
            // FIX: Use 'raw' for RGBA values in defaults for better compatibility.
            'default' => ['raw' => 'rgba(255, 255, 255, 0.3)'],
        ];
        ...
        $this->controls['button_hover_background'] = [
            ...
            'type'    => 'color',
            // FIX: Use 'raw' for RGBA values in defaults.
            'default' => ['raw' => 'rgba(255, 255, 255, 0.2)'],
        ];
    }
    ...
    public function render() {
        ...
        // FIX: Adopt the robust ID generation from the working example.
        ...
        $video_url  = ! empty( $settings['video_file']['url'] ) ? $settings['video_file']['url'] : 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4';
        ...
        // Color settings
        // FIX: Access the correct keys from the color picker settings array.
        $accent_color    = $settings['primary_accent_color']['hex'] ?? '#3b82f6';
        $text_color      = $settings['text_color']['hex'] ?? '#ffffff';
        $slider_track    = $settings['slider_track_color']['raw'] ?? 'rgba(255, 255, 255, 0.3)';
        $chapter_dot_color= $settings['chapter_dot_color']['hex'] ?? '#ffffff';
        $btn_hover_bg    = $settings['button_hover_background']['raw'] ?? 'rgba(255, 255, 255, 0.2)';
        ...
        echo "<style>
            ...
            #" . esc_attr($root_id) . " .snn-video-container { position: relative; background-color: #000; overflow: hidden; border-radius: 0.5rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); height: var(--player-height); }
            #" . esc_attr($root_id) . " .snn-video-container video { width: 100%; height: 100%; display: block; border-radius: 0.5rem; object-fit: cover; }
            #" . esc_attr($root_id) . " .snn-video-container:fullscreen { width: 100vw; height: 100vh; max-width: 100%; border-radius: 0; }
            #" . esc_attr($root_id) . " .snn-controls-overlay { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: flex-end; opacity: 0; transition: opacity 0.3s ease-in-out; background-image: linear-gradient(to top, rgba(0,0,0,0.7), transparent); }
            ...
            #" . esc_attr($root_id) . " .snn-controls-bar-container { padding: 0.6rem 1rem; }
            #" . esc_attr($root_id) . " .snn-progress-container { position: relative; margin-bottom: 0.5rem; }
            #" . esc_attr($root_id) . " .snn-progress-tooltip { position: absolute; background-color: #1f2937; color: var(--text-color); font-size: 0.75rem; border-radius: 0.25rem; padding: 0.25rem 0.5rem; top: -2rem; pointer-events: none; opacity: 0; transition: opacity 0.2s; white-space: nowrap; transform: translateX(-50%); }
            ...
            #" . esc_attr($root_id) . " .snn-controls-left, #" . esc_attr($root_id) . " .snn-controls-right { display: flex; align-items: center; gap: 0.5rem; }
            #" . esc_attr($root_id) . " .snn-control-button { background: none; border: none; color: var(--text-color); padding: 0.5rem; border-radius: 9999px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background-color 0.2s; }
            ...
            #" . esc_attr($root_id) . " .snn-volume-container:hover .snn-volume-slider { width: 5rem; opacity: 1; }
            #" . esc_attr($root_id) . " .snn-volume-slider { margin-left: 0.5rem; }
            #" . esc_attr($root_id) . " .snn-time-display { font-size: 0.875rem; font-family: monospace, sans-serif; }
            #" . esc_attr($root_id) . " .snn-slider { -webkit-appearance: none; appearance: none; width: 100%; height: 5px; background: var(--slider-track-color); cursor: pointer; border-radius: 5px; transition: height 0.2s ease; }
            #" . esc_attr($root_id) . " .snn-slider:hover { height: 8px; }
            #" . esc_attr($root_id) . " .snn-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 16px; height: 16px; background: var(--primary-accent-color); border-radius: 50%; cursor: pointer; border: 2px solid var(--text-color); transition: transform 0.2s ease; }
            #" . esc_attr($root_id) . " .snn-slider:hover::-webkit-slider-thumb { transform: scale(1.1); }
            #" . esc_attr($root_id) . " .snn-slider::-moz-range-thumb { width: 16px; height: 16px; background: var(--primary-accent-color); border-radius: 50%; cursor: pointer; border: 2px solid var(--text-color); }
            ...
            <div class="snn-controls-overlay">
                <div class="snn-controls-bar-container">
                    <div class="snn-progress-container">
                        ...
                        <input type="range" class="snn-slider snn-progress-bar" min="0" max="100" step="0.1" value="0">
                    ...
                    <div class="snn-controls-bar">
                        <div class="snn-controls-left">
                            ...
                            <div class="snn-volume-container">
                                ...
                                <input type="range" class="snn-slider snn-volume-slider" min="0" max="1" step="0.05" value="1" aria-label="Volume">
                            </div>
        ...
        ?>
        <script>
        // FIX: The main problem was running the script inside 'DOMContentLoaded'.
        ...
        (() => {
            ...
            let isSeeking = false, inactivityTimer, lastVolume = 1;
            ...
            const updateMuteIcon = () => { if(muteBtn) muteBtn.innerHTML = video.muted || video.volume === 0 ? ICONS.volumeMute : ICONS.volumeHigh; };
            ...
            // Add keyboard shortcuts only when interacting with the player
            videoContainer?.addEventListener('focusin', () => document.addEventListener('keydown', handleKeydown));
            videoContainer?.addEventListener('focusout', () => document.removeEventListener('keydown', handleKeydown));
            ...
            updateProgressBarFill(progressBar);
            updateProgressBarFill(volumeSlider);
            showControls(); // Show controls briefly on load
        })();
        </script>
        <?php
