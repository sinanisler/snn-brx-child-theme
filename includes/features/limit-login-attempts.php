...
// Single cached fetch of security options — avoids repeated get_option calls per request
function snn_get_security_options() {
    static $options = null;
    if ( $options === null ) {
...
function snn_is_limit_login_enabled() {
    $options = snn_get_security_options();
...
// Pure expiry check — no DB writes, safe to call in loops
function snn_is_block_expired( $block_data ) {
    return ( current_time('timestamp') - $block_data['blocked_at'] ) > ( snn_get_reset_time() * HOUR_IN_SECONDS );
}

// Check if IP is currently blocked — also cleans up expired blocks on login attempts
function snn_is_ip_blocked($ip) {
    ...
    if ( snn_is_block_expired( $blocked_ips[$ip] ) ) {
...
// Block login page if IP is blocked (wp-login.php)
...
// Block XML-RPC requests from blocked IPs — login_init does not fire for XML-RPC
function snn_check_blocked_ip_xmlrpc( $user, $username, $password ) {
    if ( !snn_is_limit_login_enabled() ) {
        return $user;
    }
    ...
    if ( snn_is_ip_blocked( $ip ) ) {
        return new WP_Error(
            'too_many_retries',
            __( 'Your IP address has been temporarily blocked due to too many failed login attempts.', 'snn' )
        );
    }
    return $user;
}
add_filter( 'authenticate', 'snn_check_blocked_ip_xmlrpc', 1, 3 );
...
// Uses snn_is_block_expired() directly to avoid DB writes in a loop
function snn_get_blocked_ips_count() {
    $blocked_ips = snn_get_blocked_ips();
    $active_blocks = 0;
    foreach ( $blocked_ips as $ip => $data ) {
        if ( !snn_is_block_expired( $data ) ) {
            $active_blocks++;
        }
    }
