 ...
 * Parent ID Tag
 * ----------------------------------------
 * Usage: {parent_id} or {parent_id:top_level}
 * Description:
 * - {parent_id} returns the direct parent ID (one level up)
 * - {parent_id:top_level} returns the top-level parent (grandparent) ID
...
function render_parent_id_tag( $tag, $post, $context = 'text' ) {
    ...
    // Check if top_level parameter is used
    $get_top_level = strpos( $tag, ':top_level' ) !== false;

    if ( $get_top_level ) {
        // Get the top-level parent (grandparent)
        if ( $post->post_parent ) {
    ...
    // Default behavior: Return only the direct parent (one level up)
    if ( $post->post_parent ) {
...
function render_parent_id_tag_in_content( $content, $post, $context = 'text' ) {
    // Handle both {parent_id} and {parent_id:top_level}
    if ( strpos( $content, '{parent_id' ) !== false ) {
        // Replace {parent_id:top_level} first to avoid conflicts
        if ( strpos( $content, '{parent_id:top_level}' ) !== false ) {
            $parent_id = render_parent_id_tag( '{parent_id:top_level}', $post, $context );
            $content   = str_replace( '{parent_id:top_level}', $parent_id, $content );
        }
