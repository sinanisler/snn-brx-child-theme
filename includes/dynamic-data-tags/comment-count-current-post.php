...
add_filter('bricks/dynamic_tags_list', 'register_comment_count_current_post_tag');
function register_comment_count_current_post_tag($tags) {
    $tags[] = [
...
function get_comment_count_current_post_data() {
    // Get current post ID
    $post_id = get_the_ID();
...
add_filter('bricks/dynamic_data/render_tag', 'render_comment_count_current_post_tag', 20, 3);
function render_comment_count_current_post_tag($tag, $post, $context = 'text') {
    // Ensure that $tag is a string before processing.
    if (is_string($tag)) {
        if ($tag === '{comment_count_current_post}') {
            return get_comment_count_current_post_data();
        }
    }
    ...
    if (is_array($tag)) {
        foreach ($tag as $key => $value) {
            if (is_string($value) && $value === '{comment_count_current_post}') {
                $tag[$key] = get_comment_count_current_post_data();
            }
        }
        return $tag;
    }
...
add_filter('bricks/dynamic_data/render_content', 'render_comment_count_current_post_tag_in_content', 20, 3);
add_filter('bricks/frontend/render_data', 'render_comment_count_current_post_tag_in_content', 20, 2);
function render_comment_count_current_post_tag_in_content($content, $post, $context = 'text') {
    ...
    if (strpos($content, '{comment_count_current_post}') !== false) {
        $value = get_comment_count_current_post_data();
