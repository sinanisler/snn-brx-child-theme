...
/**
 * SNN Bricks Double Text/Textarea Repeaters & Dynamic Data Integration
 * ----------------------------------------------------
 *
 * This file extends Bricks Builder with custom query types and dynamic data tags for DOUBLE TEXT and DOUBLE TEXTAREA custom field repeaters.
 *
 * Features implemented:
 *
 * 1. **Custom Query Types for Double Text/Textarea Repeaters**
 *    - Registers new query types for each double_text and double_textarea custom field marked as a repeater.
 ...
 *    - When inside a double text/textarea repeater query loop, tags output the current item's value 1 or value 2.
 *    - Outside the loop, tags fallback to first item values.
 *    - Automatically parses JSON array format ["value1", "value2"].
 *
 * 4. **Global Variables for Template Usage**
 *    - Sets global variables for current double text/textarea repeater item and field name.
 *    - Cleans up globals after loop ends for safety.
 *
 * 5. **Bricks Hooks Used**
 *    - `bricks/setup/control_options`: Register custom query types.
 *    - `bricks/query/run`: Provide double text/textarea repeater data for custom query types.
 *    - `bricks/query/loop_object`: Set global context for current double text/textarea repeater item.
...
// ================================================
// REGISTER DOUBLE TEXT/TEXTAREA REPEATER QUERY TYPES
// ================================================

add_filter('bricks/setup/control_options', function($control_options) {
    $custom_fields = get_option('snn_custom_fields', []);
    foreach ($custom_fields as $field) {
        // Register both double_text and double_textarea fields that are repeaters
        if (!empty($field['repeater']) && isset($field['type']) && in_array($field['type'], ['double_text', 'double_textarea'])) {
            $type_key = 'snn_repeater_' . $field['name'];
            $field_type_label = ($field['type'] === 'double_text') ? 'Double Text' : 'Double Textarea';
            $label = 'SNN ' . $field_type_label . ' Repeater ' . $field['label'];
...
// ================================================
// PROVIDE DOUBLE TEXT/TEXTAREA REPEATER DATA FOR QUERY LOOP
// ================================================

add_filter('bricks/query/run', function($results, $query_obj) {
    $custom_fields = get_option('snn_custom_fields', []);
    foreach ($custom_fields as $field) {
        // Handle both double_text and double_textarea repeater fields
        if (!empty($field['repeater']) && isset($field['type']) && in_array($field['type'], ['double_text', 'double_textarea'])) {
            $type_key = 'snn_repeater_' . $field['name'];
...
add_filter('bricks/query/loop_object', function($loop_object, $loop_key, $query_obj) {
    ...
    if (strpos($query_obj->object_type, 'snn_repeater_') === 0) {
        // Check if this is a double_text or double_textarea repeater
        $field_name = str_replace('snn_repeater_', '', $query_obj->object_type);
        $custom_fields = get_option('snn_custom_fields', []);
        
        foreach ($custom_fields as $field) {
            if ($field['name'] === $field_name && isset($field['type']) && in_array($field['type'], ['double_text', 'double_textarea'])) {
...
// ================================================
// REGISTER CUSTOM DYNAMIC DATA TAGS FOR DOUBLE TEXT/TEXTAREA
// ================================================
...
add_filter('bricks/dynamic_tags_list', function($tags) {
    ...
    foreach ($custom_fields as $field) {
        // Register both double_text and double_textarea repeater fields
        if (!empty($field['repeater']) && isset($field['type']) && in_array($field['type'], ['double_text', 'double_textarea'])) {
...
// ================================================
// RENDER INDIVIDUAL DOUBLE TEXT/TEXTAREA TAGS
// ================================================
...
add_filter('bricks/dynamic_data/render_tag', function($tag, $post, $context = 'text') {
    ...
    // Get the value for this double field
    $value = snn_get_double_field_value($field_name, $index, $post);
...
/**
 * Check if a field is a double_text or double_textarea repeater field
 */
function snn_is_double_field_repeater($field_name) {
    $custom_fields = get_option('snn_custom_fields', []);
    foreach ($custom_fields as $field) {
        if ($field['name'] === $field_name && 
            !empty($field['repeater']) && 
            isset($field['type']) && 
            in_array($field['type'], ['double_text', 'double_textarea'])) {
            return true;
        }
    }
    return false;
...
/**
 * Parse a double text/textarea value (JSON string or array) and return array
 * 
 * @param mixed $value JSON string like '["value1","value2"]' or already decoded array
...
add_action('bricks/query/after_loop', function($query_obj) {
    if (strpos($query_obj->object_type, 'snn_repeater_') === 0) {
        // Check if this was a double text or double textarea repeater
        $field_name = str_replace('snn_repeater_', '', $query_obj->object_type);
        if (snn_is_double_field_repeater($field_name)) {
            global $snn_current_double_text_item, $snn_current_double_text_field;
