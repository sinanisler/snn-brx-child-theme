...
document.addEventListener('DOMContentLoaded', function() {
  ...
  animateElements.forEach(element => {
    ...
    if (options.scrub === 'true') {
      scrubValue = true;
    } else if (options.scrub === 'false') {
      scrubValue = false;
    } else if (!isNaN(parseFloat(options.scrub))) {
      scrubValue = parseFloat(options.scrub);
    } else {
      scrubValue = false; 
    }

    gsap.set(splitText ? element.children : element, {
      x: options.x ? parseInt(options.x) : 0,
      y: options.y ? parseInt(options.y) : 50, 
      opacity: options.o ? parseFloat(options.o) : 0, 
    ...
    gsap.fromTo(
      ...
      },
      {
        ...
        scale: 1,
        ...options.endStyles, 
        duration: parseFloat(options.duration) || 1, 
        scrollTrigger: {
